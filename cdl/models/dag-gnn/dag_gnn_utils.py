...
def preprocess_adj(adj):
    ...
    adj_normalized = (torch.eye(adj.shape[0], device=device) + (adj.transpose(0,1)))
...
def preprocess_adj_new1(adj):
    device = adj.get_device()
    adj_normalized = torch.inverse(torch.eye(adj.shape[0], device=device) - adj.transpose(0,1))
    return adj_normalized
